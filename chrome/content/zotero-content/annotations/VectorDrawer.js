...
(function ($, R, _) {
	...
	setHTML:function(){
		...
		deleteShape: function(id){
			if(this._activeShapes[id]){
				this._activeShapes[id].remove();
				this._activeShapes[id].type="skip";
			}
		},
		...
		scale: function(x) {
			...
			$("svg").width($("svg").width()*x);
			$("svg").height($("svg").height()*x);
					 ...
					 if (o && o.newO) {
						//replace the older values with the new ones
                         _.each(["scale", "con", "args", "x", "y", "width", "height",
							...
							self._canvas.elm.trigger("shapeChanged",[self.exportShapes]);
